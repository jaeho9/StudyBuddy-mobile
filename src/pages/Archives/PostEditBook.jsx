...
const PostEditBook = ({ navigation, route }) => {
  ...
  const handleSearch = async () => {
    try {
      ...
      const api_url = `https://openapi.naver.com/v1/search/book?query=${encodeURI(
        book
      )}`;
      const options = {
        headers: {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FFF" }}>
      <Header
        ...
        right={"선택"}
        rightClick={() =>
          navigation.navigate("PostEdit", { id: post_id, book: selectBook })
        }
      />
